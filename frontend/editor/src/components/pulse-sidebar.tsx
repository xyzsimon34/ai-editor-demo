    ...
    <>
      ...
      >
        ...
        <div className={'flex items-center justify-between border-b border-border bg-muted/50 p-4'}>
          ...
          <button onClick={onToggle} className={'rounded-md p-1 transition-colors hover:bg-muted'}>
            <X className={'size-4'} />
          </button>
        </div>
        ...
        <div className={'space-y-3 border-b border-border p-4'}>
            ...
            <div className={'flex flex-col gap-2'}>
              ...
              >
                ...
                <div className={'flex-1'}>
                  ...
                  <div className={'text-xs text-muted-foreground'}>{'Get context and background info'}</div>
                </div>
              </label>
              <label
                className={'flex cursor-pointer items-center gap-2 rounded-md p-2 transition-colors hover:bg-muted'}
              >
                ...
                <div className={'flex-1'}>
                  ...
                  <div className={'text-xs text-muted-foreground'}>{'Polish and improve text quality'}</div>
                </div>
              </label>
            </div>
          </div>
          ...
          >
            {isLoading ? (
              <>
                <div
                  className={'mr-2 size-4 animate-spin rounded-full border-2 border-background border-t-transparent'}
                />
                {'Processing...'}
              </>
            ) : (
        ...
        <div className={'flex-1 space-y-4 overflow-y-auto p-4'}>
          ...
          {!suggestions && !error && !isLoading && (
            <div className={'flex h-full flex-col items-center justify-center px-4 text-center'}>
              ...
              <p className={'mb-2 text-sm text-muted-foreground'}>{'No suggestions yet'}</p>
              <p className={'text-xs text-muted-foreground/70'}>
                {'Write something in the editor and click '}&quot;{'Get Suggestions'}&quot;
          ...
          {suggestions && !isLoading && (
            <div className={'space-y-4'}>
              {Object.entries(suggestions.suggestions).map(([agent, content]) => (
                <div key={agent} className={'overflow-hidden rounded-lg border border-border bg-muted/30'}>
                  <div className={'flex items-center gap-2 border-b border-border bg-muted/50 px-3 py-2'}>
                    <div className={'size-2 rounded-full bg-primary'} />
                    <span className={'text-xs font-semibold uppercase tracking-wide text-primary'}>{agent}</span>
                  </div>
                  <div className={'p-3'}>
